Shard Core version 0.13.2 is now available from:

  <https://download.shard.org/shard-0.13.2.1/>
...
for previous versions of Shard Core was to raise the default minimum
relay fee.

Shard Core 0.13.2 will have a strict maximum size on the mempool. The
...
Shard Core 0.13.2 also introduces new default policy limits on the length and
...
entirely in Shard Core 0.13, and it is at this time undecided whether the
...
- Adding `dbcache=100` in shard.conf
...
- `shard-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `shard-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of shards they think they are spending, and by having full nodes refuse to
...
- **Script versioning:** Segwit makes it easy for future soft forks to allow
  ...
  currently being researched by Bitcoin and Shard Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Shard
client to Shard Core 0.13.2 to signal for segwit support. When a
...
Since Shard Core 0.10.0, nodes have defaulted to only relaying and
...
- `shard-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `shard-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
