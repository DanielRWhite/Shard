...
*Setup instructions for a Gitian build of Shard Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Shard
...
- [Building Shard Core](#building-shard-core)
...
Clone the git repositories for shard and Gitian.
...
in the shard repository under 'Fetch and create inputs' to install sources which require
...
To build Shard Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the shard repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/shard/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/shard-project/shard
...
For example:
```bash
URL=https://github.com/thrasher-/shard.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit shard=${COMMIT} --url shard=${URL} ../shard/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit shard=${COMMIT} --url shard=${URL} ../shard/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit shard=${COMMIT} --url shard=${URL} ../shard/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the shard git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../shard/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/shard
SIGPATH=/some/root/path/shard-detached-sigs

./bin/gbuild --url shard=${BTCPATH},signature=${SIGPATH} ../shard/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/shard-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/shard-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/shard-osx-build.assert
```
...
[shard-project/gitian.sigs.ltc](https://github.com/shard-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
